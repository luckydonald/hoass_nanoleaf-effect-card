import { mdiDelete, mdiDragHorizontalVariant, mdiPencil, mdiPlus } from '@mdi/js';
...
type SupportsFeature = (hass: HomeAssistant, context: LovelaceCardFeatureContext) => boolean;
...
const SUPPORTS_FEATURE_TYPES: Record<UiFeatureTypes, SupportsFeature | undefined> = {
    ...
    'climate-swing-horizontal-modes': supportsClimateSwingHorizontalModesCardFeature,
...
const CUSTOM_FEATURE_ENTRIES: Record<string, CustomCardFeatureEntry | undefined> = {};
...
export const supportsFeaturesType = (hass: HomeAssistant, context: LovelaceCardFeatureContext, type: string) => {
    ...
    protected render() {
        ...
        return html`
            ...
            ${supportedFeaturesType.length > 0
                ? html`
                      <ha-button-menu fixed @action=${this._addFeature} @closed=${stopPropagation}>
                          ...
                          ${types.map(
                              (type) => html`
                                  <ha-list-item .value=${type}> ${this._getFeatureTypeLabel(type)} </ha-list-item>
                              `
                          )}
