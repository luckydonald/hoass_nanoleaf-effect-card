         ...
         <div class="buttons-container ${this._config.button_style?.compact ? 'compact-grid' : ''}">
           ${effects
             .map((effect) => {
               ...
               const buttonStyle: ButtonStyle = { ...(this._config.button_style ?? {}), ...(effect.button_style ?? {}) };
    ...
    if (typeof el.setConfig !== 'function') {
      el.setConfig = function (cfg: CardConfig) {
        ...
        try {
          ...
          const self = this as HTMLElement & { attachShadow?: (opts: ShadowRootInit) => ShadowRoot; shadowRoot?: ShadowRoot | null };
    ...
    return Object.values(ha.states)
      .filter(
        (entity) =>
          ...
          (entity.attributes['supported_color_modes'] as string[]).find((mode) => ['hs', 'rgb', 'xy'].indexOf(mode) !== -1),
      )
      .filter((entity) => {
        ...
        if (mode === 'color_temp') {
          ...
          if (!Array.isArray(attrs['supported_color_modes']) || !(attrs['supported_color_modes'] as string[]).includes('hs')) {
            return false;
          }
          if (
