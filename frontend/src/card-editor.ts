  ...
  attachEventListeners(): void {
    ...
    if (entityPicker && !(entityPicker as BoundElement)._nanoleaf_bound) {
      ...
      entityPicker.addEventListener('value-changed', (e: Event) => {
        ...
        const value = customEv.detail?.value ?? (e.target as HTMLInputElement).value ?? (entityPicker as Element & { value: string }).value;
  ...
  render(): void {
    ...
    this.shadowRoot!.querySelectorAll('.effect-picker').forEach((picker) => {
      ...
      picker.addEventListener('value-changed', (e: Event) => {
        ...
        const input = this.shadowRoot!.querySelector(`.effect-name-input[data-index="${idx}"]`) as HTMLInputElement | null;
        if (input) {
          input.value = customEv.detail.value ?? '';
          const isValid = !customEv.detail.value || (this._effectList && this._effectList.includes(customEv.detail.value));
  ...
  attachEffectsListeners(): void {
    ...
    this.shadowRoot!.querySelectorAll('.color-input').forEach((input) => {
      ...
      input.addEventListener('input', (e: Event) => {
        ...
        const effects = [...(this._config.effects ?? [])];
        const colors = [...(effects[effectIndex].colors ?? (effects[effectIndex].color ? [effects[effectIndex].color!] : ['#CCCCCC']))];
