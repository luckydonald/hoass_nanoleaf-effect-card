...
class NanoleafEffectCardEditor extends HTMLElement {
  ...
  setConfig(config: CardConfig): void {
    ...
    incomingEffects.forEach((inc, i) => {
      ...
      if (!btnStyle.color_display && p.button_style?.color_display) {
    ...
    const colorInputs = colors
      .map((color, colorIndex) => /* html */ `
       <div style="display:flex; align-items:center; gap:6px;">
        ...
        />
        <button
id="effect-${effectIndex}-delete-color-${colorIndex}"
class="icon-button delete-color"
data-effect-index="${effectIndex}"
data-color-index="${colorIndex}"
title="Remove color"
>
